  ...
  const mainWorldScript = function() {
    ...
    class HTMLMediaPlayer {
      constructor(media) {
        ...
        this.onTimeUpdate = throttle(this._onTimeUpdate.bind(this), 1e3)
        ...
        this.timeIntervalId = setInterval(this.onTimeUpdate, 5e3)
      ...
      onSeeked() {
        ...
        // Immediately announce updated time
        this._onTimeUpdate()
      }

      _onTimeUpdate() {
        // Get buffered time ranges to use in autosync
        const { buffered } = this.media
        const timeRanges = [...Array(buffered.length).keys()].map(idx => [
          Math.floor(buffered.start(idx) * SEC2MS),
          Math.ceil(buffered.end(idx) * SEC2MS)
        ])
      ...
      onWaiting(event) {
        ...
        switch (playerSettings.autoSyncMode) {
          case 'continuous':
            method = this.autoSyncContinuous
            break
          default:
            return
        }
      ...
      /** Attempt to autosync by continuously seeking until we're at the desired position. */
      autoSyncContinuous() {
        let timeoutId = null

        const stopSyncing = () => {
          this.media.removeEventListener('playing', stopSyncing, false)
        ...
        this._endWaiting = stopSyncing
        this.media.addEventListener('playing', stopSyncing, false)
