...
var showSubmenu,showSaveAsDialog,closeTabAfter;
...
function initialize()
{
    ...
    function(object)
    {
        ...
        title += (lazyLoadType == "1") ? "Scroll:" : "Shrink:";
        ...
        switch (message.type)
        {
            ...
            case "scriptLoaded":
                ...
                chrome.tabs.sendMessage(sender.tab.id,{ type: "performAction",
                                                        ...
                                                        swapdevices: tabSaveParams[sender.tab.id].swapdevices },checkError);
    ...
    function(win)
    {
        ...
        function(tabs)
        {
            ...
            if (menuaction == 0)
            {
                ...
                nextAction(menuaction,saveditems,togglelazy,extractsrcurl,(selectedTabIds.length > 1),externalsave,swapdevices);
            }
            else if (menuaction == 1)
            {
                ...
                nextAction(menuaction,saveditems,togglelazy,extractsrcurl,true,externalsave,swapdevices);
            }
            else
            {
                selectedTabIds.length = 0;
                ...
                nextAction(menuaction,saveditems,togglelazy,extractsrcurl,false,externalsave,swapdevices);
            }
...
function nextAction(menuaction,saveditems,togglelazy,extractsrcurl,multiplesaves,externalsave,swapdevices)
{
    ...
    if (menuaction == 0)
    {
        if (selectedTabIds.length > 0)
        {
            ...
            function (tab)
            {
                performAction(currentTabId,menuaction,saveditems,togglelazy,extractsrcurl,multiplesaves,externalsave,swapdevices);
            });
        }
        else if (useAutomation) 
        {
            window.setTimeout(  /* allow time to catch Alt+C after saving all tabs */
            function()
            {
                if (!cancelSave)
                {
    ...
    else if (menuaction == 1)
    {
        if (listedURLs.length > 0)
        {
            ...
            function()
            {
                ...
                nextAction(menuaction,saveditems,togglelazy,extractsrcurl,multiplesaves,externalsave,swapdevices);
            },urlListTime*1000);
...
function performAction(tabId,menuaction,saveditems,togglelazy,extractsrcurl,multiplesaves,externalsave,swapdevices)
{
    ...
    function (tab)
    {
        if (chrome.runtime.lastError == null)  /* in case tab does not exist */
        {
            ...
            else
            {
                ...
                else if (tabSaveStates[tab.id] == -2)  /* script loaded */
                {
                    ...
                    chrome.tabs.sendMessage(tab.id,{ type: "performAction",
                                                     ...
                                                     swapdevices: swapdevices },checkError);
                }
